...
def search_with_evidence(query, *, legacy_search, connection_factory, limit=10,
    ...
    pool_limit = min(200, max(40, limit * 4))
    terms = _terms(query, evidence_terms)
    if not query.strip():
        return {"results": [], "evidence_version": 1, "candidate_ids": []}
    legacy_rows, _ = legacy_search(query, limit=pool_limit, mode=mode,
    ...
    try:
        ...
        for term in terms:
            ...
            for row in conn.execute(sql, [*filters, f"%{escaped}%", pool_limit]):
                pool[row[0]] = tuple(row)
    finally:
        conn.close()
    results = []
    ...
    results.sort(key=lambda r: (len(r["body_matched_terms"]), r["relevance"], r["timestamp"], str(r["id"])), reverse=True)
    return {"results": results[:limit], "evidence_version": 1,
            "candidate_ids": [r["id"] for r in results], "pool_limit_per_term": pool_limit}
